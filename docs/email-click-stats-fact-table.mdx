...
| dim_campaign_id | bigint | Identifier for the campaign in which the email message is configured. | [campaigns](/dimensions-tables#campaigns) |
| dim_event_date_id | bigint | Date when the email was accessed by the customer. | [date](/dimensions-tables#date) |
| dim_event_time_id | bigint | Time when the email was accessed by the customer. | [time](/dimensions-tables#time) |
| dim_event_user_id | bigint | Identifier of the customer associated with the email click. | [users](/dimensions-tables#users-users) |
