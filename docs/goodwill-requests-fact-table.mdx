...
| Column Name | Data Type | Description | Linked Table | Availability for Export in the [Goodwill Requests](/docs/goodwill-requests-standard-export-template) Template |
| :--- | :--- | :--- | :--- | :--- |
...
| dim_event_program_id | bigint | Identifier for the loyalty program name against which the goodwill points have been allocated. | [program](/dimensions-tables#program) | Yes; Dimension Name: Event Program |
| dim_event_user_id | bigint | Identifier for the user associated with the event. It is an internally assigned value by Capillary for user identification. | [users](/dimensions-tables#users-users) | Yes; Dimension Name: User Id |
...
| dim_points_category_id | bigint | Gives the category name against which the goodwill points have been allocated. Possible values are - Regular points, Trackers, Promised points, Alternate currencies, and External trigger based points. | [points_category](/dimensions-tables#points-category) | Yes; Dimension Name: Points Category |
| dim_request_created_date_id | bigint | Date when the goodwill request was created. | [date](/dimensions-tables#date) | Yes; Dimension Name: Date |
| dim_request_updated_date_id | bigint | Date when the goodwill request was updated. | [date](/dimensions-tables#date) | Yes; Dimension Name: Date |
